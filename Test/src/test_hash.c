...
static char const * test_hash_any_2n(void)
{
	unsigned char out[2*SPHINCS_BYTES];

	/* in1 test case */
	hash_any_2n(out, in1, 0);

	mu_assert("Test case failed for IN1.", array_cmp(out, in1_hashed_512, SPHINCS_BYTES));

	/* in2 test case */
	hash_any_2n(out, in2, 43);

	mu_assert("Test case failed for IN2.", array_cmp(out, in2_hashed_512, SPHINCS_BYTES));
...
void run_test_hash(void)
{
	...
	mu_run_test(test_hash_any_2n, "test_hash_any_2n()");
